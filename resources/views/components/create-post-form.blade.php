<form action="{{ route('posts.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
    ...
    <textarea name="content"
        rows="6"
        class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 px-4"
    ...
    <div class="mt-4">
        ...
        <input type="text" name="language"
            class="mt-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 px-4"
    ...
    <div class="mt-4">
        ...
        <input type="file" name="images[]" accept="image/*" multiple
            ...
            hover:file:bg-violet-100 px-4">
    </div>

    <div class="mt-4">
        <label class="block text-sm font-medium text-gray-700">Vidéo</label>
        <input type="file" name="videos[]" accept="video/mp4,video/mov,video/avi" multiple
            class="mt-1 block w-full text-sm text-gray-500
            file:mr-4 file:py-2 file:px-4
            file:rounded-full file:border-0
            file:text-sm file:font-semibold
    ...
    <div class="mt-4">
        ...
        <textarea name="code_snippet"
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono px-4"
            rows="4"
            placeholder="Ajouter un extrait de code (optionnel)"></textarea>
    </div>
